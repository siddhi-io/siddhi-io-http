...
import org.wso2.siddhi.extension.http.source.util.HttpSourceUtil;
...
class HttpConnectorRegistry {
    ...
    boolean createServerConnector(String listenerUrl, String context, String sourceId,
            ...
            Boolean> registeredListenerAuthentication, ConfigReader sourceConfigReader) {
        ...
        if (!registeredServerConnectors.containsKey(baseUrl)) {
            ...
            HttpMessageProcessor httpMessageProcessor = new HttpMessageProcessor(baseUrl);
            ...
            try {
                ...
                registeredListenerAuthentication.remove(listenerUrl);
                throw new HttpSourceAdaptorRuntimeException("Failed to initialized server for URL " + listenerUrl +
                        " in " + sourceId + "connection refuse in " + sourceId);
            }
            return true;
        } else {
